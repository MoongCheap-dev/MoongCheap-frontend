...
⚠️ **팀 확인 필요** — 커밋마다 전체 typecheck를 돌릴지, pre-push로 옮길지. 지금은 코드가 거의 없어 문제없지만 나중에 재검토할 지점이다.
...
- ⚠️ **`test` 제거는 근거 불명확 — 팀 확인 필요.** 테스트 코드를 쓰기 시작하면 쓸 타입이 없어진다(`chore`나 `feat`에 섞게 됨). `src/tests/` 폴더는 이미 있다. 테스트 도입 계획이 있다면 `test`를 되살리는 편이 낫다.
- ⚠️ **`build`, `ci`, `perf`, `revert` 제거도 근거 불명확 — 팀 확인 필요.** 특히 `ci`는 이 저장소에 GitHub Actions 워크플로가 있어 수정할 일이 생긴다. 현재는 `chore`로 커밋해야 한다.
